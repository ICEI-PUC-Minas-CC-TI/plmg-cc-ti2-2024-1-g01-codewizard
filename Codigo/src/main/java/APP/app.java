	...
	public static void main(String[] args) {
        ...
        before((request, response) -> {
         ...
         // Rotas que não requerem autenticação
            List<String> publicRoutes = Arrays.asList("/login", "/login.html", "/register-user", "/api/recognize-text");
        ...
        post("/api/recognize-text", (req, res) -> {
            req.attribute("org.eclipse.jetty.multipartConfig", new MultipartConfigElement("/temp"));

            try (InputStream is = req.raw().getPart("image").getInputStream()) {
                VisionService visionService = new VisionService("ad001fd6aad8480dafaf928427fa8ec1", "https://codewizard.cognitiveservices.azure.com/");
                String result = visionService.recognizeTextFromImage(is);
                return result;
            } catch (Exception e) {
                e.printStackTrace();
                res.status(500);
